...
public class AVLTree<E extends Comparable<? super E>> {
    ...
    public E deleteMin() {
        ArrayList<AvlNode> result = new ArrayList<>();
        root = deleteMin(root, result);
        return (result.size() > 0) ? result.get(0).value : null;
    }
    ...
    public void printTree(String label) {
        ...
        String treeString = makeString(root, 0);
        System.out.println(treeString);
    }

    private E find(E value, AvlNode node) {
        if (node == null) {
            return null;
        }
    ...
    private AvlNode deleteMin(AvlNode node, ArrayList<AvlNode> result) {
        if (node == null) {
            return null;
        }
    ...
    private String makeString(AvlNode node, int level) {
        if (node == null) {
            return "";
        }
        StringBuilder buffer = new StringBuilder();

        buffer.append(makeString(node.right, level + 1));

        String indent = "   ";
        for (int i = 0; i < level; i++) {
            buffer.append(indent);
        }
        buffer.append(node.value).append("(").append(height(node)).append(")\n");
        buffer.append(makeString(node.left, level + 1));
